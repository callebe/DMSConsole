...
É comum haver mais de um painel eletrônico em um autocarro, de modo a tornar visível de qualquer lado de uma via as informações dos painéis. Os painéis são comumente distribuídos no autocarro pelo para-brisa e as janelas laterais, como pode ser visto na Figura (\ref{fig:ModeloAutocarro}). 
...
A lista de elementos 'Painel' presente como um atributo nos elementos 'Destino' são usados para especificar como as informações de destino e linha do autocarro devem ser exibidos nas diferentes dimensões de painéis eletrônicos. Assim é possível exibir no painel frontal o número, nome da linha e destino do autocarro. Mas em um painel lateral menor exibir apenas o número da linha. O objeto 'Painel' ainda possui uma lista de objetos 'Pagina', com atributos de espaçamento e tempo de afixação. Cada 'Pagina' por sua vez possuem elementos 'Campos' que descrevem os textos exibidos, e suas respectivas fontes e efeitos. A Figura \ref{fig:EstruturaDeDados} apresenta esta estrutura de dados.
...
\begin{figure}[H]
	\centering
	\captionsetup{width=0.7\textwidth, font=footnotesize, textfont=bf}	
	\includegraphics[width=0.7\linewidth]{Images/RevisaoDeLiteratura/IndicadoresDeDestinoDMS/EstruturaDeDadosGID.eps}
