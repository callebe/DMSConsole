...
Para cada página da interface há uma camada \textit{touch} e a camada \textit{frame} associadas. Quanto maior for o número de páginas necessárias para oferecer as funcionalidade da interface ao usuário, mais extenso será o  trabalho de desenvolver os dois tipos de camadas para cada página. Para tornar a tarefa de desenvolvimento menos penosa, a interface foi pensada de modo a reduzir o número de páginas, e reunir o máximo de ferramentas para o usuário em uma mesma página, sem afetar a visibilidade e o uso das mesmas. 
...
Na página de Configiguração de \textit{Display} o usuário tem  acesso às configurações de \textit{Blacklight}, \textit{Screen Saver} e \textit{Buzzer Tocu Sound}. O \textit{Blacklight}, ou retroiluminação, é intensidade da fonte de iluminação de fundo do \textit{Display}. Uma intensidade alta de torna de \textit{Blacklight} torna o conteúdo mostrado no \textit{Display} mais visível ao usuário em um ambiente bastante iluminado. Porém em um ambiente mais escuro um nível alto de  \textit{Blacklight} pode incomodar o usuário é causar mais rapidamente um cansaço visual. Na página de configuração do \textit{Display} é possível selecionar o nível do  \textit{Blacklight} do \textit{Display},e ainda definir para dois períodos do dia a intensidade de  \textit{Blacklight} desejado.
