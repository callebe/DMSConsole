...
Para que a aplicação DMSConsole seja capaz te gerenciar a rede de painéis eletrônicos conectada ao console e atender as requisições feitas através da interface de usuário, é desenvolvida a biblioteca DMSDisplay.  Esta biblioteca contem as funções capazes de efetuar o reconhecimento e configurações dos dispositivos da rede de indicadores de destino. Para tal esta biblioteca se utiliza de uma interface de comunicação serial emulada e dois \textit{timers} do sistema Linux. Estes \textit{timers}  são uteis para realizar algumas rotinas de configurações e manter o procedimento cíclico de atualização dos painéis. 
...
Os pinos da GPIO do \textit{Raspberry Pi Zero W} possuem um nível de tensão de 0V á 3,3V, e consequentemente qualquer interface de comunicação estabelecido estará neste limite de tensão. Já que o UART-485 opera como tensões diferenciais em uma faixa além da oferecida pelo \textit{Raspberry}, se faz necessário utilizar um conversor UART TLL / UART-485. Neste projeto o conversor utilizado é o CI SN65HVD308 - TI (\cite{485}), o qual apresenta no lado TLL 4 pinos; porta de envio, recepção, habilitação de envio e aterramento. No lado 485 há apenas duas portas diferencias A e B. A Figura \ref{fig:ConversorUARTTTL485} apresenta o esquemático da conexão entre este conversor e o \textit{Raspberry}.
...
O ID (\textit{Identificador}) de destino e de origem são dados específicos de cada dispositivo, normalmente o  console tem por padrão o ID 0x00 e o painel com as maiores dimensões da rede tem o ID 0x01. Os comandos 'Tipo' e 'Dados' são de acordo com o tipo de mensagem a ser enviada pelo protocolo, na seção \ref{subsection:MensageConsolePanel} é especificado os valores para tais comandos.
...
Já no caso de um teste de memória RAM e Flash, todos os painéis da rede ao receberem esta mensagem passam a checar suas memorias, e cada uma envia sequencialmente uma mensagem ACK caso o teste ocorra com sucesso, ou NACK caso tenham encontrado algum problema durante o teste. 
...
\begin{figure}[H]
	...
	\includegraphics[width=0.85\linewidth]{Images/SoftwareDMSConsole/TouchDisplayInterface.eps}
...
\begin{figure}[H]
	...
	\begin{subfigure}[t]{.5\textwidth}
		\centering
		\captionsetup{width=0.9\textwidth, font=footnotesize, textfont=bf}	
		\includegraphics[width=0.9\linewidth]{Images/SoftwareDMSConsole/FrameworkPanelSettings.eps}
	...
	\begin{subfigure}[t]{.5\textwidth}
		\centering
		\captionsetup{width=0.9\textwidth, font=footnotesize, textfont=bf}	
		\includegraphics[width=0.9\linewidth]{Images/SoftwareDMSConsole/FrameworkPanelSettingsError.eps}
	...
	\begin{subfigure}[t]{.5\textwidth}
		\centering
		\captionsetup{width=0.9\textwidth, font=footnotesize, textfont=bf}	
		\includegraphics[width=0.9\linewidth]{Images/SoftwareDMSConsole/FrameworkPanelSettingsConfirm.eps}
...
Na página de Configiguração de \textit{Display} o usuário tem  acesso às configurações de \textit{Blacklight}, \textit{Screen Saver} e \textit{Buzzer Tocu Sound}. O \textit{Blacklight}, ou retro-iluminação, é intensidade da fonte de iluminação de fundo do \textit{Display}. Uma intensidade alta de torna de \textit{Blacklight} torna o conteúdo mostrado no \textit{Display} mais visível ao usuário em um ambiente bastante iluminado. Porém em um ambiente mais escuro um nível alto de  \textit{Blacklight} pode incomodar o usuário é causar mais rapidamente um cansaço visual. Na página de configuração do \textit{Display} é possível selecionar o nível do  \textit{Blacklight} do \textit{Display},e ainda definir para dois períodos do dia a intensidade de  \textit{Blacklight} desejado.

O \textit{Screen Saver} é o recurso que reduz o \textit{Blacklight} para zero quando o \textit{Display} esta ocioso. Na  página de configuração do \textit{Display} é possível definir um valor de até 900 segundos para que o tempo máximo de ociosidade para que o \textit{Display} não entre em \textit{Screen Saver}, porém se este valor for 0 o \textit{Screen Saver} é desativado.

Por ultimo existe o recurso de \textit{Buzzer Tocu Sound} que é um recurso sonoro que informa ao usuário quando uma tecla \textit{touch} foi pressionada. O tempo de emissão sonora pode ser configurado nesta página, sendo a unidade definida em 10 milésimos de segundos. 

\vspace{3mm}
\begin{figure}[H]
	\centering
	\captionsetup{width=0.9\textwidth, font=footnotesize, textfont=bf}	
	\includegraphics[width=0.9\linewidth]{Images/SoftwareDMSConsole/FrameworkDisplaySettings.eps}
