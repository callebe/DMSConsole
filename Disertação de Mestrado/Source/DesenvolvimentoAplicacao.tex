...
Assim através da interface de comunicação emulada via \textit{bit bang}, utilizando os pinos 20 23 e 24 da GPIO, a comunicação entre console e painéis é feita no padrão LDESIGN. Neste protocolo de comunicação qualquer mensagem enviada ou recebida por um dispositivo da rede segue o seguinte formato:
...
O comando 'STREAM ID' tem dois formato diferentes, quando a mensagem é no sentido painel-console 'STREAM ID' é composto com 1 byte ASCII com o caractere 'A' quando a informação é no formato ASCII, ou 'B' quando é no formato BITMAP. Já quando a mensagem é no sentido console-painel o comando 'STREAM ID' é composto por 5 bytes com a mensagem 'ASCII' quando a mensagem esta no formato ASCII, e caso contrario a mensagem devera ser no formato BITMAP. 
